...
gb = sd.groupby(['Position']).Value.mean().reset_index().sort_values(by=['Value'], ascending = False)
gb['Value'] = gb['Value'].astype(int)

print(gb.head())
print(gb.tail())
gb.to_excel("AvrageValuePerPosition-" + datetime.now().strftime("%d-%m-%Y %H-%M-%S") + ".xlsx")
